...
fn generate_bits<R: Rng>(rng: &mut R, len: usize) -> (Vec<bool>, BitVec<u8>, (u16, Vec<u8>), (u16, [u8; 64]), ArrayKey, VecKey) {
    ...
    (vec_bool, bitvec, (len as u16, vec_u8.clone()), (len as u16, u8_array), ArrayKey{depth: len as u16, index: u8_array}, VecKey{depth: len as u16, index: vec_u8.clone()})
}
...
fn benchmark(c: &mut Criterion) {
    ...
    for key_len in [1, 32, 66, 128, 200, 279, 512] {
        let mut test_data: Vec<(Vec<bool>, BitVec<u8>, (u16, Vec<u8>), (u16, [u8; 64]), ArrayKey, VecKey, [GoldilocksField; 8])> = Vec::with_capacity(SAMPLES_PER_SIZE);
        for _ in 0..SAMPLES_PER_SIZE {
            let (vec_bool_key, bitvec_key, vec_u8_key, u8_array_key, array_struct, vec_struct) = generate_bits(&mut rng, key_len);
            let value = generate_value(&mut rng);
            test_data.push((vec_bool_key, bitvec_key, vec_u8_key, u8_array_key, array_struct, vec_struct, value));
        }

        group.bench_function(format!("vecbool_insert/{}", key_len), |b| {
            b.iter(|| {
                let mut map = HashMap::new();
                for (vec_bool_key, _, _, _, _, _, value) in &test_data {
        ...
        group.bench_function(format!("bitvec_insert/{}", key_len), |b| {
            b.iter(|| {
                let mut map = HashMap::new();
                for (_, bitvec_key, _, _, _, _, value) in &test_data {
        ...
        group.bench_function(format!("vecu8_insert/{}", key_len), |b| {
            b.iter(|| {
                let mut map = HashMap::new();
                for (_, _, vec_u8_key, _, _, _, value) in &test_data {
        ...
        group.bench_function(format!("u8array_insert/{}", key_len), |b| {
            b.iter(|| {
                let mut map = HashMap::new();
                for (_, _, _, u8_array_key, _, _, value) in &test_data {
        ...
        group.bench_function(format!("array_struct_insert/{}", key_len), |b| {
            b.iter(|| {
                let mut map = HashMap::new();
                for (_, _, _, _, array_struct, _, value) in &test_data {
        ...
        group.bench_function(format!("vec_struct_insert/{}", key_len), |b| {
            b.iter(|| {
                let mut map = HashMap::new();
                for (_, _, _, _, _, vec_struct, value) in &test_data {
        ...
        for (vec_bool_key, bitvec_key, vec_u8_key, u8_array_key, key_struct, vec_struct, value) in &test_data {
        ...
        group.bench_function(format!("vecbool_lookup/{}", key_len), |b| {
            b.iter(|| {
                for (vec_bool_key, _, _, _, _, _, _) in &test_data {
        ...
        group.bench_function(format!("bitvec_lookup/{}", key_len), |b| {
            b.iter(|| {
                for (_, bitvec_key, _, _, _, _, _) in &test_data {
        ...
        group.bench_function(format!("vecu8_lookup/{}", key_len), |b| {
            b.iter(|| {
                for (_, _, vec_u8_key, _, _, _, _) in &test_data {
        ...
        group.bench_function(format!("u8array_lookup/{}", key_len), |b| {
            b.iter(|| {
                for (_, _, _, u8_array_key, _, _, _) in &test_data {
        ...
        group.bench_function(format!("array_struct_lookup/{}", key_len), |b| {
            b.iter(|| {
                for (_, _, _, _, array_struct, _, _) in &test_data {
                    black_box(arraystruct_map.get(black_box(array_struct)));
                }
            });
        });

        group.bench_function(format!("vec_struct_lookup/{}", key_len), |b| {
            b.iter(|| {
                for (_, _, _, _, _, vec_struct, _) in &test_data {
