...
fn main() {
    let (vk, pf) = VerificationKey::setup::<true>(|builder, [this, root, mesg]| {
        ...
        let contract_storage_digest_index = this
            .elements
            .iter()
            .flat_map(|&v| builder.split_le(v, 64))
            .collect::<Vec<_>>();
        let contract_storage_slot_index = hash_zero
            .elements
            .iter()
            .flat_map(|&v| builder.split_le(v, 64))
            .collect::<Vec<_>>();
        let contract_storage_digest = builder.add_virtual_hash();
        let contract_storage_digest_path = MerkleProofTarget {
            siblings: builder.add_virtual_hashes(256),
        };
        let contract_storage_slot = builder.add_virtual_target_arr::<8>();
        let contract_storage_slot_path = MerkleProofTarget {
            siblings: builder.add_virtual_hashes(256),
        };
        let contract_storage_slot_new = builder.add_virtual_target_arr::<8>();
        let update_digest = vec![(hash_zero, contract_storage_slot_new)].iter().fold(
            hash_zero,
            |left, (key, value)| {
        ...
        let message = builder.hash_n_to_hash_no_pad::<PoseidonHash>(
            [hash_zero.elements, update_digest.elements].concat(),
        );
        ...
        builder.verify_merkle_proof::<PoseidonHash>(
            contract_storage_digest.elements.into(),
            &contract_storage_digest_index,
            root,
            &contract_storage_digest_path,
        );
        builder.verify_merkle_proof::<PoseidonHash>(
            contract_storage_slot.into(),
            &contract_storage_slot_index,
            contract_storage_digest,
            &contract_storage_slot_path,
        );
        ...
        paths
            .into_iter()
            .flat_map(|v| v.siblings)
            .chain(hashes.into_iter())
            .flat_map(|v| v.elements)
            .chain(slots.into_iter().flatten())
            .chain(targets)
            .collect()
    });
    let addr = vk.address();
    ...
    for i in 0..16 {
        ...
        let contract_storage_slot_new = from_fn(|i| {
            if i == 0 {
                contract_storage_slot[i].add_one()
            } else {
                contract_storage_slot[i]
            }
        });
        ...
        let mesg = PoseidonHash::two_to_one(
            Hash::ZERO,
            updates.iter().fold(Hash::ZERO, |left, (key, value)| {
