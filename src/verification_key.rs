...
#[derive(Debug, Clone)]
pub struct VerificationKey(plonky2::plonk::circuit_data::VerifierOnlyCircuitData<Config, 2>);
impl VerificationKey {
    pub fn setup(f: impl FnOnce(&mut CircuitBuilder<GoldilocksField, 2>, [HashOutTarget; 3]) -> Vec<Target>) -> (Self, impl Fn([Hash; 3], Vec<GoldilocksField>) -> Result<Proof>) {
        ...
        (Self(verifier_only), move |hashes, values| {
    ...
    pub fn verify(&self, root: Hash, mesg: Hash, proof: Proof) -> Result<()> { (VerifierCircuitData { verifier_only: self.0.clone(), common: Self::common() }).verify(ProofWithPublicInputs { proof, public_inputs: [self.address().elements, root.elements, mesg.elements].concat() }) }
    pub fn address(&self) -> Hash { PoseidonHash::hash_no_pad(&self.0.constants_sigmas_cap.0.iter().map(|v| v.elements).flatten().chain(self.0.circuit_digest.elements).collect::<Vec<_>>()) }
